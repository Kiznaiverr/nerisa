module.exports = {
   ...
   run: async (m, {
      ...
      try {
         ...
         const medias = json.data.result.map(v => ({
            url: v.url,
            type: v.type
         }))
         if (medias.length === 1) {
            const file = medias[0]
            return conn.sendFile(m.chat, file.url, file.type === 'video' ? Func.filename('mp4') : Func.filename('jpg'), `🍟 *Process* : ${((new Date - old) * 1)} ms`, m)
         } else {
