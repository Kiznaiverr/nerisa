...
module.exports = {
  ...
  run: async (m, { conn, usedPrefix, command, text, Func }) => {
    try {
      ...
      let chatJid = Object.entries(global.db.chats)
        .filter(([jid, _]) => jid.endsWith(".net"))
        .map(([jid, _]) => jid);
      ...
      if (id.length == 0)
        return conn.reply(
          m.chat,
          Func.texted("bold", `🚩 Error, ID does not exist.`),
          m,
        );
        ...
        try {
          ...
          if (text) {
            ...
            conn.reply(
              ...
              Func.texted(
                "bold",
                `🚩 Media / text not found or media is not supported.`,
              ),
              m,
            );
        } catch (e) {
