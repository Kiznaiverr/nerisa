...
module.exports = async (conn, ctx, database) => {
  ...
  try {
    ...
    let isOwner = [
      conn.decodeJid(conn.user.id).replace(/@.+/, ""),
      env.owner,
      ...setting.owners,
    ]
    ...
    let participants = m.isGroup
      ? groupMetadata
        ? conn.resolveLid(groupMetadata.participants)
        : []
      : [] || [];
    let adminList = m.isGroup
      ? participants
    ...
    let isBotAdmin = m.isGroup
      ? adminList.includes(conn.user.id.split`:`[0] + "@s.whatsapp.net")
      : false;
    ...
    if (setting.debug && !m.fromMe && isOwner)
      conn.reply(m.chat, Func.jsonFormat(m), m);
    if (!m.fromMe && m.isGroup && groupSet.antibot && !isOwner && isBotAdmin) {
      if (m.isBot || /interactiveMessage|buttonsMessage/.test(m.mtype))
        return conn
          .reply(
            m.chat,
            Func.texted(
              "italic",
              `⚠ Bot lain tidak diperbolehkan di grup ini..`,
            ),
            m,
          )
          .then(async () => {
    ...
    cron.schedule(
      ...
      async () => {
        ...
        Object.values(global.db.users)
          .filter((v) => v.limit < env.limit && !v.premium)
          .map((v) => (v.limit = env.limit));
    ...
    if (setting.antispam && spam) {
      ...
      if (spam.status === "ban_temp")
        return conn.reply(
          m.chat,
          `🚩 [ ${spam.data.ban_times} / ${spam.data.max_ban} ] You are temporarily banned for *${Func.toTime(spam.data.cooldown * 1000)}*.`,
          m,
        );
      if (spam.status === "ban_permanent")
        return conn.reply(
          m.chat,
          `🚩 [ ${spam.data.ban_times} / ${spam.data.max_ban} ] You've been warned many times! *You are permanently banned!*`,
          m,
        );
    ...
    if (isCommand) {
      ...
      if (!m.isGroup && env.blocks.some((no) => m.sender.startsWith(no)))
        return conn.updateBlockStatus(m.sender, "block");
      if (setting.pluginDisable?.includes(name)) return;
      if (setting.error?.includes(command))
        return conn.reply(
          m.chat,
          Func.texted(
            "bold",
            `🚩 Command _${(prefix ? prefix : "") + command}_ disabled.`,
          ),
          m,
        );
      if (
        (m.fromMe && m.isBot) ||
        /broadcast|newsletter/.test(m.chat) ||
        /Edit/.test(m.mtype)
      )
        return;
      ...
      if (
        ...
        groupSet.adminonly &&
        !isAdmin &&
        !["groupinfo", "link", "me"].includes(name)
      )
        return;
      if (
        !m.isGroup &&
        !["owner", "anonymous", "anonymous-send_contact"].includes(name) &&
        chats &&
        !isPrem &&
        !isOwner &&
        !users.banned &&
        ...
        return conn
          .sendMessageModify(
            m.chat,
            `⚠️ The bot is currently in group mode. To use it in private chats, please join the group first or upgrade to the premium package by sending *${prefixes[0]}premium.*`,
            m,
            {
              largeThumb: true,
      ...
      if (
        ...
        users &&
        (users.banned || new Date() - users.ban_temporary < env.timeout)
      )
        return;
      if (
        m.isGroup &&
      ...
      if (
        ...
        !isOwner &&
        text &&
        new RegExp("\\b" + setting.toxic.join("\\b|\\b") + "\\b").test(
          text.toLowerCase(),
        )
      ) {
        return conn
          .reply(
            m.chat,
            `⚠️ You have violated the *Terms and Conditions* of bot usage by using prohibited keywords. As punishment for your violation, your account has been banned.`,
            m,
          )
          .then(() => {
            users.banned = true;
      ...
      if (plugin.premium && !isPrem)
        return conn.reply(m.chat, global.status.premium, m);
      if (plugin.limit && users.limit < 1)
        return conn.reply(
          m.chat,
          `⚠️ You reached the limit and will be reset at 00.00\n\nTo get more limits upgrade to premium plans.`,
          m,
        );
      ...
      if (plugin.admin && !isAdmin && !(plugin.ownerBypass && isOwner))
        return conn.reply(m.chat, global.status.admin, m);
      if (plugin.private && m.isGroup)
        return conn.reply(m.chat, global.status.private, m);
      if (plugin.game && !setting.game)
        return conn.reply(m.chat, global.status.gameInactive, m);
      if (plugin.register && !users.registered)
        return conn.reply(
          m.chat,
          `To use this command, please register first with the command *${prefix}reg gender + age*`,
          m,
        );
      try {
        await plugin.run(m, {
        ...
        if (plugin.limit && !plugin.game && users.limit > 0) {
          ...
          if (users.limit >= limit) {
            ...
            return conn.reply(
              m.chat,
              `⚠️ Your limit isn't enough to use this feature.`,
              m,
            );
          }
        }
      ...
      for (const event of loadEvt) {
        ...
        if (
          !m.isGroup &&
          setting.groupmode &&
          !["anonymous_chat", "chatbot"].includes(name) &&
          !isPrem
        )
          return conn
            .sendMessageModify(
              ...
              m,
              {
                ...
                thumbnail: fs.readFileSync(
                  `./lib/assest/image/acces-denied.jpg`,
                ),
                url: setting.link,
              },
            )
        ...
        try {
          ...
          if (
            ...
            return conn.reply(
              m.chat,
              `⚠️ You reached the limit and will be reset at 00.00\n\nTo get more limits upgrade to premium plan.`,
              m,
            );
        } catch (e) {
          return conn.reply(m.chat, e.toString(), m);
