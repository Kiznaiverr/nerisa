...
const connect = async () => {
  ...
  conn.once("connect", async (x) => {
    ...
    global.db = {
      ...
      statistic: {},
      sticker: {},
      ...((await system.database.fetch()) || {}),
    };
    /** save db */
    await system.database.save(global.db);
    /** write log */
    if (x && typeof x === "object" && x.message)
  ...
  conn.once("prepare", async (x) => {
    ...
    /** initialize sholat scheduler */
    try {
      global.sholatScheduler = require("./lib/sholat-scheduler")(
        conn,
        env,
        Func,
      );
    } catch (e) {
      console.error("Failed to initialize Sholat Scheduler:", e.message);
    }
    ...
    setInterval(
      async () => {
        if (global?.db?.setting?.autobackup) {
          ...
          fs.writeFileSync(
            env.database + ".json",
            JSON.stringify(global.db, null, null),
            "utf-8",
          );
          await conn.sock.sendFile(
