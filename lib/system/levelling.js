const { Config: env, Function: Func } = require('@znan/wabot')
module.exports = class Levelling {
   /**
    * Growth rate
    * `2.0`
    */
   growth = 2.0
   /**
    * get XP range at specified level
    * @param {Number} level 
    * @param {Number} multiplier 
    */
   xpRange = (level, multiplier = env.multiplier || 1) => {
      if (level < 0) throw new TypeError('level cannot be negative value')
      level = Math.floor(level)
      let min = level === 0 ? 0 : Math.round(Math.pow(level, this.growth) * multiplier) + 1
      let max = Math.round(Math.pow(++level, this.growth) * multiplier)
      return {
         min,
         max,
         xp: max - min
      }
   }
   /**
    * get level by xp
    * @param {Number} xp 
    * @param {Number} multiplier 
    */
   findLevel = (xp, multiplier = env.multiplier || 1) => {
      if (xp === Infinity) return Infinity
      if (isNaN(xp)) return NaN
      if (xp <= 0) return -1
      let level = 0
      do level++
      while (this.xpRange(level, multiplier).min <= xp)
      return --level
   }
   /**
    * is able to level up?
    * @param {Number} level 
    * @param {Number} xp 
    * @param {Number} multiplier 
    */
   canLevelUp = (level, xp, multiplier = env.multiplier || 1) => {
      if (level < 0) return false
      if (xp === Infinity) return true
      if (isNaN(xp)) return false
      if (xp <= 0) return false
      return level < this.findLevel(xp, multiplier)
   }
}
Func.updateFile(require.resolve(__filename))