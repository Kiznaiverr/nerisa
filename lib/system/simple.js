...
const { generateWAMessageFromContent, generateWAMessage, areJidsSameUser } = require('@whiskeysockets/baileys')
...
module.exports = conn => {
      ...
      let location = {
         ...
         message: {
            "imageMessage": {
               "mimetype": "image/jpeg",
               "caption": caption,
               "jpegThumbnail": fs.readFileSync(`./lib/assets/images/thumbnail.jpg`)
            }
         }
      }
   ...
   conn.resolveLid = arr => {
      return arr.map(obj => {
         const id = obj.jid || obj.phoneNumber
         const lid = obj.lid || obj.id
         const admin = obj.admin ?? null
         return { id, lid, admin }
      })
   }
   /**
    ...
    * @returns 
    */
   conn.getAdmin = participants => participants?.filter(i =>
