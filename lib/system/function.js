...
Func.greeting = () => {
   const hour = parseInt(moment.tz(process.env.TZ).format('HH'), 10)
...
Func.similarity = (str1, str2) => {
   ...
   const pairs1 = pairs(str1)
   const pairs2 = pairs(str2)
...
Func.checkLevelUp = async (users, conn, m) => {
   const Levelling = require('./levelling');
   const levelling = new Levelling();
   let oldLevel = users.level || 0;
   users.level = levelling.findLevel(users.exp);
   if (users.level > oldLevel) {
      ...
      let caption = `乂  *A U T O - L E V E L U P*\n\n`;
      caption += `   ◦  *Congratulations!* You leveled up to ${users.level}\n`;
